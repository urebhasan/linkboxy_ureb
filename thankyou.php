...
//if(!empty($_GET['tx'] && !empty($_GET['cm']))){
if(!empty($_GET['cm'])){
	$time = date('Y-m-d H:i:s');
	$stmt = $common->mysqli->prepare("UPDATE users SET package = '$_GET[cm]' WHERE id = '$_SESSION[reg_id]' LIMIT 1");
	$stmt->execute();
    $stmt->close();
	$stmt = $common->mysqli->prepare("INSERT into payments(user_id,package,trax,created_at) VALUES('$_SESSION[reg_id]','$_GET[cm]','$_GET[tx]','$time')");
	$stmt->execute();
                        ...
                        <div class="row tab-back">
                            <div class="col-md-12 p-0">
                                ...
                                <div class="profile-back">
									<h4>Thank You ! Your payment is recieved successfully.</h4><br>
									<?php 
									if(isset($_SESSION['user_id'])){
										$text = "<p>You can now begin to use our service by <a href=".$config['site_url'].">clicking here</a>!</p>";
									}else{
										$text = "<p>Congratulations! Please check your email and follow the link to activate your account.</p>";
										$text .= "<p>Once you've followed the activation link on your email, please, <a href=".$config['site_url'].">login here</a>!</p>";
									}
									?>
									<p><?php echo $text; ?></p>
                                </div>
